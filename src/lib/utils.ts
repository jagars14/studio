...
import { differenceInYears, differenceInMonths, differenceInDays, addDays, subDays, setYear, isPast } from 'date-fns';
...
export function generateReproductiveEvents(animals: Animal[]): ReproductiveEvent[] {
  ...
  animals.forEach(animal => {
    ...
    if (animal.birthDate) {
        ...
        let nextBirthday = setYear(birthDate, new Date().getFullYear());
        if (isPast(nextBirthday)) {
            nextBirthday = addDays(nextBirthday, 365); // Simplificado, no maneja bisiestos perfectamente pero es suficiente
        }
        events.push({
